...
func sqlImageSearch(searchTerm, url string) []TFIDFScore {
	...
	err = tx.QueryRow("SELECT term_id FROM image_terms WHERE word = ?;", searchTerm).Scan(&termID)
	if err != nil {
	...
	var numDocs int
	err = tx.QueryRow("SELECT count(*) FROM image_hits WHERE term_id = ?;", termID).Scan(&numDocs)
	if err != nil {
	...
	err = tx.QueryRow("SELECT COUNT(url_id) FROM image_urls;").Scan(&docs_in_C)
	if err != nil {
	...
	rows, err = tx.Query(` SELECT image_terms.word, image_urls.url_title, image_urls.url, image_hits.src_url, image_hits.alt_text, image_urls.word_count, image_hits.term_count
			FROM image_hits
			INNER JOIN image_urls ON image_hits.url_id = image_urls.url_id
			INNER JOIN  image_terms on image_terms.term_id = image_hits.term_id
			WHERE image_hits.term_id = ?;`, termID)
